...
static struct dst_addr_status dst_mac_status(struct rte_mbuf *m, unsigned portid); //Chcks if the dst_addr exists in the MAC table. If it does not, then it adds it to the table. 1 if it exits, 2 if it exists and is coding capable, and 0 if not.
