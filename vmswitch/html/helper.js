	...
	if($("#" + "cpu_mem_info").length != 0) //Check if in index.php by looking for an ID that only exists in index.php
	{
		...
		function loadStatus()
		{
			...
			$.ajax({
				url: 'utilities.php',
				type: 'POST',
				data: {'func': 'isSwitchOn'},
				success: function(data) { 
					//Check if switch process is running.
					var switch_process_on = parseInt(data);
					if(switch_process_on == 1)
					{
						$.ajax({
							url: 'utilities.php',
							type: 'POST',
							...
							success: function(data) { 
								...
								$('#cpuinfo').text("CPU: "+(Math.floor(cpu_use*100/8))+"%");
							}
						});
						...
						$.ajax({
							...
							success: function(data) { 
								...
								{
									if(append_linkrows == 1)
									{
										$('#link_table tbody').append("<tr id=link"+portid+">"+table_row+"</tr>");
									}
									var table_row = "";
									for(heading_index in table_headings)
									{
										...
										{
											if(table_heading == "ipackets")
											{
												rx_total += parseInt(table_val);
											}
											else if(table_heading == "opackets")
											{
												tx_total += parseInt(table_val);
											}
											table_row += "<td>"+table_val+"</td>";
										}
									}
									pos_offset = parseInt(data.indexOf(";",data.indexOf("oerrors:", pos_offset)));
									rowData += ("<tr>"+table_row+"</tr>");
									$('#rx').text("RX: "+rx_total + " pkts");
									$('#tx').text("TX: "+tx_total + " pkts");
								}
								$('#link_table tbody').html(rowData);
								append_linkrows = 0;
							}
						});
						//$("#link_table tbody").html("");
					}
					else if(switch_process_on == 0)
					{
